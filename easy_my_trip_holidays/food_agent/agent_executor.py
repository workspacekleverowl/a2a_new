...
class FoodAgent(BaseModel):
    """Food agent that returns food information"""

    async def invoke(self, task: str) -> str:
        # Simple parsing
        cuisine_match = re.search(r"(\w+) restaurant", task)
        cuisine = cuisine_match.group(1).strip() if cuisine_match else "Any"
        ...
        response = {
            "restaurants": [
                {
                    "name": f"Dummy {cuisine.capitalize()} Restaurant in {location}",
